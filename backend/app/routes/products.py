...
async def fetch_products(
    ...
    query = select(Product).options(joinedload(Product.category))  # Eager load the category relationship
    ...
    if max_price:
        query = query.filter(Product.price <= max_price)  # Corrected from >= to <= for max_price
    if keyword:
...
@product_router.get("/products_id/{product_id}", response_model=ProductResponse)
